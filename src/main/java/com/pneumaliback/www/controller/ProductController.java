...
public class ProductController {
        ...
        public ResponseEntity<?> filter(@RequestParam(required = false) Long categoryId,
                ...
                return ResponseEntity
                                .ok(productService.findWithFilters(category, brand, size, season, minPrice, maxPrice,
                                                pageable));
        }
        ...
        public ResponseEntity<?> create(
                ...
                try {
                        ...
                        Long profileId = profileIdStr != null && !profileIdStr.isBlank() ? Long.parseLong(profileIdStr)
                                        : null;
                        Long diameterId = diameterIdStr != null && !diameterIdStr.isBlank()
                                        ? Long.parseLong(diameterIdStr)
                                        : null;
                        ...
                        CreateProductRequest request = new CreateProductRequest(
                                        ...
                                        season != null && !season.isBlank()
                                                        ? com.pneumaliback.www.enums.TireSeason.valueOf(season)
                                                        : null,
                                        vehicleTypeId,
                                        tireConditionId,
                ...
                try {
                        ...
                        if (image != null && !image.isEmpty()) {
                                ...
                                if (existingProduct.getImageUrl() != null && !existingProduct.getImageUrl().isBlank()) {
                                        String oldFilePath = storageService
                                                        .extractFilePathFromUrl(existingProduct.getImageUrl());
                                        if (oldFilePath != null) {
                        ...
                        Long categoryId = categoryIdStr != null && !categoryIdStr.isBlank()
                                        ? Long.parseLong(categoryIdStr)
                                        : null;
                        Boolean active = activeStr != null && !activeStr.isBlank() ? "true".equalsIgnoreCase(activeStr)
                                        : null;
        ...
        @GetMapping("/latest")
        @Operation(summary = "Derniers produits actifs", description = "Retourne les pneus récemment ajoutés")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Liste récupérée", content = @Content(mediaType = "application/json", schema = @Schema(implementation = Product.class))),
                        @ApiResponse(responseCode = "500", description = "Erreur interne", content = @Content(mediaType = "application/json"))
        })
        public ResponseEntity<?> latest(@RequestParam(defaultValue = "3") int limit) {
                int safeLimit = Math.max(1, Math.min(limit, 12));
                return ResponseEntity.ok(productService.latestActive(safeLimit));
        }
